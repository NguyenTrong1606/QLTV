...
//				DrawHieuChinhDocGia();
				...
				bar(XDG[6]+50 + textwidth(ThongBao), 675, w-20, 675+textheight(ThongBao));
...
void ItemDocGiaEvent(){
	if(mx > XDG[0] && mx < XDG[btnTatCaDocGia.isChoose?5:6] && my > 290-8 && my < 290+13*40-8){
		
		if(curItemDG != GetItemDocGiaPosition(my)){
			if(curItemDG != -1){
				// khoi phuc item
				setfillstyle(SOLID_FILL, BG_COLOR);
				bar(XDG[0], 290 + curItemDG*40 - 8, XDG[btnTatCaDocGia.isChoose?5:6], 290+(curItemDG+1)*40-8);
...
void DrawDanhSachDocGia(){
	...
	outtextxy((XDG[0]+XDG[5])/2-textwidth(DocGiaTitle)/2, 50, DocGiaTitle);
	
	settextstyle(BOLD_FONT, HORIZ_DIR, 3);
	setcolor(TIPS);
	outtextxy(20, 135, "Sap xep theo:");
...
void ClearRightLayoutDocGia(){
	setfillstyle(SOLID_FILL, BG_COLOR);
	bar(XDS[6]+15, 0 , w, 800);
}

void ClearLayoutListDocGia(){
	setfillstyle(SOLID_FILL, BG_COLOR);
	bar(XDG[0]-5, 220-5, XDG[5]+5, 980);
}	

void ClearLayoutListDocGiaQuaHan(){
	setfillstyle(SOLID_FILL, BG_COLOR);
	bar(XDG[0]-5, 220-5, XDG[6]+5, 980);
}

void DrawBorderDSDocGia(){	
	setlinestyle(SOLID_LINE, 0, 3);
	setcolor(BORDER_COLOR);
	rectangle(XDG[0], 220, XDG[5], 820);
	line(XDG[0], 220 + 25 + 2*20, XDG[5], 220 + 25 + 2*20);
	for(int i=0; i < 5; i++){
		line(XDG[i+1], 220, XDG[i+1], 820);
	}
	...
	for(int i=0; i < 5; i++){
		setbkcolor(BG_COLOR);
		outtextxy(XDG[i] + (XDG[i+1]-XDG[i])/2 - textwidth(ch[i])/2, 240, ch[i]);
	}
}
...
void DrawBorderDSDocGiaQuaHan(){
	setlinestyle(SOLID_LINE, 0, 3);
	setcolor(BORDER_COLOR);
	rectangle(XDG[0], 220, XDG[6], 820);
	line(XDG[0], 220 + 25 + 2*20, XDG[6], 220 + 25 + 2*20);
	for(int i=0; i < 6; i++){
		line(XDG[i+1], 220, XDG[i+1], 820);
	}
	
	char ch[7][20] = {"Ma the", "Ho", "Ten", "Phai", "Trang thai the", "So ngay muon"};
	settextstyle(SANS_SERIF_FONT, HORIZ_DIR, 2);
	for(int i=0; i < 6; i++){
		setbkcolor(BG_COLOR);
		outtextxy(XDG[i] + (XDG[i+1]-XDG[i])/2 - textwidth(ch[i])/2, 240, ch[i]);
...
void DrawListDocGia(bool reload){
	...
	if(btnTatCaDocGia.isChoose){
		...
		outtextxy((XDG[5]+XDG[0])/2 - textwidth(chPage)/2, 895, chPage);
...
//		DSDG.GetDocGiaQuaHan(TreeDocGia);
		...
		settextstyle(BOLD_FONT, HORIZ_DIR, 2);
		outtextxy((XDG[0]+XDG[6])/2 - textwidth(chPage)/2, 895, chPage);
	...
	setcolor(TIPS);
	rectangle(XDG[6]+15, 850, w-10, 950);
	outtextxy(XDG[6]+35, 870, "Click chuot trai: Hieu chinh doc gia");
	outtextxy(XDG[6]+35, 900, "Click chuot phai: Xoa doc gia");
}
...
void DrawItemDocGia(DocGia &docgia, int i){
	...
	outtextxy(XDG[0] + 10, 290 + i*40, ch);
	outtextxy(XDG[1] + 10, 290 + i*40, docgia.ho);
	outtextxy(XDG[2] + 10, 290 + i*40, docgia.ten);
	outtextxy(XDG[3] + 30, 290 + i*40, PhaiDocGia[docgia.phai]);
	outtextxy(XDG[4] + 10, 290 + i*40, TTTDocGia[docgia.trangthai]);
}
...
void DrawItemDocGiaQuaHan(DocGia &docgia, int soNgayQH, int i){
	...
	outtextxy(XDG[5]+50, 290 + i*40, ch);
}
